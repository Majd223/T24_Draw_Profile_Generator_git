...
if Create_Annual_Profiles == 1:
    ...
    if Version == 2016:
        if Building_Type == 'Multi':
            Raw_Data = pd.read_table(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DHWDUMF.txt', header = Row_Header_Annual_2016)
        elif Building_Type == 'Single':
            Raw_Data = pd.read_table(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DHWDUSF.txt', header = Row_Header_Annual_2016)
    elif Version == 2019:
        Raw_Data = pd.read_table(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DHWDU.txt', header = Row_Header_Annual_2019)
    ...
    for i in Range:
        ...
        if Version == 2016:
            Row = Raw_Data.loc[i, '//365 day DHW Profile by number of Bedrooms'].replace('"','').replace(')','')
        elif Version == 2019:
            Row = Raw_Data.loc[i, '////////////////////////////////////////////////////////////////////////////////'].replace('"','').replace(')','')
        
        if Row[8:11] == 'DHW':
            if Name_Column == 'DHW1BR' and Building == 'SF' or Name_Column == 'DHW0BRa' and Building == 'MF':
...
if Create_Daily_Profiles == 1:
    ...
    Daily_Profiles = pd.DataFrame(columns = ['Day', 'Fixture', 'Start time (hr)', 'Duration (min)', 'Flow Rate (gpm)'])
    Day = []
    Fixture = []
    StartTime_Event = []
    Duration_Event = []
    ...
    if Version == 2016:
        if Building_Type == 'Multi':
            Raw_Data = pd.read_table(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DHWDUMF.txt', header = Row_Header_Daily_2016)
        elif Building_Type == 'Single':
            Raw_Data = pd.read_table(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DHWDUSF.txt', header = Row_Header_Daily_2016)
    elif Version == 2019:
        Raw_Data = pd.read_table(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DHWDU.txt', header = Row_Header_Daily_2019)
...
#    Raw_Data = pd.read_table(r'C:\Users\Peter Grant\Documents\Python Scripts\Hot Water Draw Profiles\CBECC-Res\DHWDU.txt', header = Row_Header_Daily)
    ...
    for i in range(len(Raw_Data)):
        ...
        if Row[0:9] == 'DHWDAYUSE':
            ...
            Row_Data = Row[2:-1].replace('(', ',').replace(')', ',').replace(' ', '').split(',')
...
if Create_Annual_Profiles == 1:
    Annual_Profile.to_csv(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'AnnualProfile' + Building + '.csv', index = False)
    print(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'AnnualProfile' + Building + '.csv')
    
if Create_Daily_Profiles == 1:
    if Version == 2016:
        Daily_Profiles.to_csv(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DailyProfiles' + Building + '.csv', index = False)
    elif Version == 2019:
        Daily_Profiles.to_csv(os.path.dirname(__file__) + os.sep + 'SourceData' + os.sep + str(Version) + os.sep + 'DailyProfiles.csv', index = False)
