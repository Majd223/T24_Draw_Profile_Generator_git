...
def Convert_Profile_SingleDay(EventBased, Day_Of_Year, Timestep, ClimateZone, SI = True, interpolate=False):
    '''
    ...
    WeatherData['T_Mains'] = 0.65 * WeatherData['T Ground'] + 0.35 * WeatherData['31-day Avg lag DB'] 

    # Create datetime index, interpolate to desired timestep
    WeatherData.loc[0, 'Timestamp'] = dt.datetime(Start.year, 1, 1, 0)
    WeatherData.loc[1:, 'Timestamp'] = WeatherData.loc[0, 'Timestamp'] + pd.to_timedelta(WeatherData.index[1:], unit = 'h')
    WeatherData = WeatherData.set_index('Timestamp')
    WeatherData = WeatherData.resample('{}S'.format(Timestep)).interpolate(method = 'linear' if interpolate else 'ffill')
    
    TimestepBased_Index = pd.date_range(Start, End, freq = '{}T'.format(Timestep/Conversions.seconds_in_minute))

    TimestepBased = pd.DataFrame(0, index = TimestepBased_Index, 
                                 columns = ['Hot Water Draw Volume (gal)', 
    ...
    for Draw in EventBased.index:
        ...
        Start_Time = Start + pd.Timedelta(EventBased.loc[Draw, 
            'Start time (hr)'], unit = 'h')
        Duration = EventBased.loc[Draw, 'Duration (min)']
        End_Time = EventBased.loc[Draw, 'Start time (hr)'] + Duration / Conversions.minutes_in_hour
        ...
        Bin_Start = int(np.floor(EventBased.loc[Draw, 'Start time (hr)']/(Timestep / (Conversions.seconds_in_minute 
                                                                                              * Conversions.minutes_in_hour))))

        Time_Start_Bin = (TimestepBased.index[Bin_Start + 1] - Start_Time).total_seconds() / Conversions.seconds_in_minute
    
        Bin_End = int(np.floor(End_Time / (Timestep / (Conversions.seconds_in_minute * Conversions.minutes_in_hour))))
        End_Timestamp = Start + pd.Timedelta(End_Time, unit = 'h')
        Time_End_Bin = (End_Timestamp - TimestepBased.index[Bin_End]).total_seconds() / Conversions.seconds_in_minute
    
        # Assign the hot water draw volume to each bin
        if Bin_End == Bin_Start:
            TimestepBased.loc[TimestepBased.index[Bin_Start], 'Hot Water Draw Volume (gal)'] += Duration * Flow_Rate
        else:
            TimestepBased.loc[TimestepBased.index[Bin_Start], 'Hot Water Draw Volume (gal)'] += Time_Start_Bin * Flow_Rate
    
        if Bin_End - Bin_Start >= 1: # If the draw doesn't end during the start bin
            TimestepBased.loc[TimestepBased.index[Bin_End], 'Hot Water Draw Volume (gal)'] += Time_End_Bin * Flow_Rate
        if Bin_End - Bin_Start >= 2: # If there are bins between start and end
            TimestepBased.loc[TimestepBased.index[Bin_Start+1] : TimestepBased.index[Bin_End-1], 
                              'Hot Water Draw Volume (gal)'] += Flow_Rate * (Timestep / Conversions.seconds_in_minute)    

    # Add mains temperature data to the output
    TimestepBased['Mains Temperature (deg F)'] = WeatherData['T_Mains']
    TimestepBased['Mains Temperature (deg F)'] = TimestepBased['Mains Temperature (deg F)'].ffill()

    # Add outdoor temperature data to the output
    TimestepBased['Outdoor Temperature (deg F)'] = WeatherData['Dry Bulb']
    TimestepBased['Outdoor Temperature (deg F)'] = TimestepBased['Outdoor Temperature (deg F)'].ffill()
    ...
    # Convert to SI units
    if SI == True:
        TimestepBased['Hot Water Draw Volume (gal)'] = TimestepBased['Hot Water Draw Volume (gal)'] * Conversions.L_in_gal
        TimestepBased['Mains Temperature (deg F)'] = (TimestepBased['Mains Temperature (deg F)'] - 32)/1.8
        TimestepBased['Outdoor Temperature (deg F)'] = (TimestepBased['Outdoor Temperature (deg F)'] - 32)/1.8
        TimestepBased = TimestepBased.rename(columns = {
...
#%%---------------------READ WEATHER DATA----------------------------------
        ...
        WeatherData = WeatherData.resample('{}S'.format(Timestep)).interpolate(method = 'ffill')
    ...
    #%%------------------DECLARE CONSTANTS------------------------------------
        ...
        for Draw in EventBased.index:
            ...
            # Gather draw data from the event-based data set
            Start_Time = Start + pd.Timedelta(EventBased.loc[Draw, 
                'Start Time of Year (hr)'], unit = 'h')
            Duration = EventBased.loc[Draw, 'Duration (min)']
            End_Time = EventBased.loc[Draw, 'Start Time of Year (hr)'] + Duration / Conversions.minutes_in_hour
            Flow_Rate = EventBased.loc[Draw, 'Hot Water Flow Rate (gpm)']
    
            # Identify the start/end bin and draw duration during each of those bins
            Bin_Start = int(np.floor(EventBased.loc[Draw, 'Start Time of Year (hr)']/(Timestep / (Conversions.seconds_in_minute 
                                                                                              * Conversions.minutes_in_hour))))
            Time_Start_Bin = (TimestepBased.index[Bin_Start + 1] - Start_Time).total_seconds() / Conversions.seconds_in_minute
        ...
        if SI == True:
            Output_File = Bldg + '_' + CZ + '_' + Wat + '_' + Prof + '_' + SDLM + '_' + CFA + '_' + Inc + '_' + Ver + '_SI.csv'
        else:
            Output_File = Bldg + '_' + CZ + '_' + Wat + '_' + Prof + '_' + SDLM + '_' + CFA + '_' + Inc + '_' + Ver + '.csv'
