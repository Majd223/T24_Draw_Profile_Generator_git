...
import time
import ast
from T24_Draw_Profile_Generator import Calculate_Fraction_HotWater, Calculate_FlowWater_Hot, Modify_Profile_SDLM
...
def Calculate_TMains(ClimateZone):
    ...
    Folder_WeatherData = os.path.join(root, 'WeatherFiles')#Folder + os.sep + 'WeatherFiles' #This states the folder that CBECC weather data files are stored in
...
def Create_Hot_Profiles(Building_Type, Profile_List, ClimateZone, Include_Faucet, Include_Shower, Include_Clothes, Include_Dish, Include_Bath, 
                              Version, T_Mains, Day_Of_Year, Temperature_Bath, Temperature_Shower, SquareFootage_Dwelling, Water, Distribution_System_Type): 
    ...
    for Day in Profile_List:
        ...
        temp = Calculate_Fraction_HotWater(Temperature_Supply_Hot_AtFixture, Temperature_Bath, Temperature_Shower, temp)
        temp = Calculate_FlowWater_Hot(temp)
        temp = Modify_Profile_SDLM(temp, SquareFootage_Dwelling, Water, Distribution_System_Type)
...
if __name__ == '__main__':
    ...
    Binned_Profiles = pd.read_csv(os.path.join(os.getcwd(), '..', 'hpwhs', 'Profiles_Binned_ByAvgElecPerDay.csv'), index_col = 0)
    for group in Binned_Profiles.index[0:1]:
        Profile_List = ast.literal_eval(Binned_Profiles.loc[group, 'Profiles'])

        Profiles = Create_Hot_Profiles('Single', Profile_List, ClimateZone, Include_Faucet, Include_Shower, Include_Clothes, Include_Dish, Include_Bath, 
                                       Version, T_Mains, Day_Of_Year, Temperature_Bath, Temperature_Shower, SquareFootage_Dwelling, Water, 
