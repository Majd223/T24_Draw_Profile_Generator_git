...
NumberBedrooms_Dwellings = [1] * 4 + [2] * 4 #The number of bedrooms in each dwelling. Is a list because multi-family buildings need multiple specifications
SquareFootage_Dwellings = [780] * 4 + [960] * 4 #The square footage of each dwelling in the building. Is a list because multi-family buildings need multiple specifications
...
WeatherData = pd.read_csv(Path_WeatherData, header = 26) #Read the weather data, ignoring the first 26 lines of header
...
def Create_Mixed_Profile_NoSDLM(Building_Type, NumberBedrooms_Dwelling, Variant, Include_Faucet, Include_Shower, Include_Clothes, 
                                Include_Dish, Include_Bath, Version, Reduce_Clothes = True): #It needs the type of building, number of bedrooms in the dwelling, and current variant of the building as inputs
    if Version == 2016:
    ...
    if Reduce_Clothes == True:
        ...
        ix = temp.loc[temp['Flow Rate (gpm)'] > 2.9615].index # 2.9615 is the upper bound defined by 1.5 IQR. See Draw Analysis in Source Data for details
        for i in ix:
            Dwelling_Profile.loc[ix, 'Flow Rate (gpm)'] = 1.2543 # Average value of CWSH draws within the 1.5 IQR range. See Draw Analysis in Source Data for details
        ix = temp.loc[temp['Duration (min)'] > 7.167].index # 7.167 is the upper bound defined by 1.5 IQR. See Draw Analysis in Source Data for details
        for i in ix:
            Dwelling_Profile.loc[ix, 'Duration (min)'] = 1.9635 # Average value of CWSH draws within the 1.5 IQR range. See Draw Analysis in Source Data for details        
...
def Create_Hot_Profile_NoSDLM(Building_Type, NumberBedrooms_Dwelling, Variant, ClimateZone, Include_Faucet, Include_Shower, Include_Clothes, 
                              Include_Dish, Include_Bath, Version, Reduce_Clothes = True): #It needs the type of building, number of bedrooms in the dwelling, and current variant of the building as inputs
...
def Calculate_Fraction_HotWater(Temperature_Supply_Hot_AtFixture, Temperature_Bath, Temperature_Shower, Data):
    ...
    for i in range(len(NumberBedrooms_Dwellings)): #For each entry in the list NumberBedrooms_Dwellings
        ...
        if Combined == 'No' and Combined_LargeBuilding == 'No': #If the user does not want all draw profiles combined into a single file, use this code to print one file for each draw profile
            ...
            if Building_Type == 'Multi': #If it's a multi-family building
                ...
                print('Finished processing {}'.format('Bldg=' + Building_Type + '_CZ=' + str(ClimateZone) + '_Wat=' + Water + '_Prof=' + str(NumberBedrooms_Dwellings[i]) + str(Variants[Variant]) + '_Inc=' + str(Included_Code) + '_Ver=' + str(Version)))
                ...
                print('Finished processing {}'.format('Bldg=' + Building_Type + '_CZ=' + str(ClimateZone) + '_Wat=' + Water + '_Prof=' + str(NumberBedrooms_Dwellings[i]) + '_Inc=' + str(Included_Code) + '_Ver=' + str(Version)))
