...
Timing insights ( Nathan's Computer):
Total time - 1.4 sec.
importing one weather file - 37 ms (3%)
all work on weather file including importing the file - 40 ms (3%)
inputting the new climate data - 800 ms (vs approx 600 ms with swifter) (42%-55%)
...
#file to convert to a new climate zone:
File = "HighRise_FullBuilding_Hot_CZ=3_SDLM=Yes_Inc=[S].csv" # mjust use double-quotations since string has singles already
...
Building_Type = Specifier_Dict['Bldg'] if 'Bldg' in Specifier_Dict else 'unspecified' #get building type to locate file
Water = Specifier_Dict['Wat'] if 'Wat' in Specifier_Dict else 'Hot' #must be a hot water draw profile

File_Location = Folder + os.sep + 'DrawProfiles' + os.sep + Building_Type + os.sep + Water + os.sep + File
File_Location = Folder + os.sep + 'DrawProfiles/Unused Profiles' + os.sep + File
...
for each in Zones_Dict: #repeat for each new zone required
    ...
    with CodeTimer('swap climate data'):
        Data['Mains Temperature (deg F)'] = Data.swifter.apply(lambda x: T_Mains[x['Day of Year (Day)']-1], axis = 1)
